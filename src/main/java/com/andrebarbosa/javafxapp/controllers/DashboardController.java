...
import com.andrebarbosa.javafxapp.utils.Utils;
import com.andrebarbosa.javafxapp.models.*;
...
public class DashboardController extends BaseController implements Initializable {
    ...
    private void populateChoiceBoxWithTextFilesOptions() {
        textFileChoicebox.setItems(FXCollections.observableArrayList(
                "Todos os Ficheiros", "Áreas Restritas", "Cartões", "Colaboradores", "Equipamentos",
    ...
    public void loadPeriodosAutorizacao() {
        ...
        List<ArrayList<String>> data = Utils
                .getDataFromFile("./src/main/resources/database/text/periodos_autorizacao.txt");
        for (ArrayList<String> d : data) {
    ...
    private void serealizeData() {
        ...
        try (FileOutputStream fos = new FileOutputStream(
                "./src/main/resources/database/serialized/" + timestamp + ".bin");
                ObjectOutputStream oos = new ObjectOutputStream(fos)) {
            oos.writeObject(empresa);
            Utils.openAlert(Utils.INFORMATION, "Data saved (serialized) with success.",
                    "Saved on file " + timestamp + ".bin.", Alert.AlertType.INFORMATION);
            updateView();
        } catch (Exception e) {
            Utils.openAlert(Utils.ERROR, e.getLocalizedMessage(), null, Alert.AlertType.ERROR);
        }
    ...
    private void deserealizeData(String filename) {
        try (FileInputStream fis = new FileInputStream("./src/main/resources/database/serialized/" + filename);
                ObjectInputStream ois = new ObjectInputStream(fis)) {
            Empresa empresaCopy = (Empresa) ois.readObject();
            Empresa.setInstance(empresaCopy);
            Utils.openAlert(Utils.INFORMATION, "Data loaded (deserialized) with success.",
                    "Loaded from file " + filename + ".", Alert.AlertType.INFORMATION);
            updateView();
        } catch (Exception e) {
            Utils.openAlert(Utils.ERROR, e.getLocalizedMessage(), null, Alert.AlertType.ERROR);
        }
